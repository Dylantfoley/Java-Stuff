...
public class DirectedWeightedGraph {
    ...
    private static final int NIL = -1; //used to initialize the predacessors
    private static final int INFINITY = 99999; //used to initialize the pathLength of all vertices
    ...
    private void dijkstra(int s){ //s = the starting vertex
        int v,c;
        for(v=0; v<n; v++){
        ...
        vertexList[s].pathLength = 0; //pathlength of source vertex = 0

        while(true){
            c = tempVertexMinPL(); //finds temporary vertex with minimum pathLength
    ...
    private int tempVertexMinPL(){ //returns temporary vertex with minimum pathLength or NIL if no temp vertex left or all temp vertex left path length == infinity
        int min = INFINITY;
        int x=NIL;
        for(int v=0; v<n; v++){
    ...
    private void findPath(int s, int v){
        int i,u;
        int [] path = new int[n]; //used to store the path from source vertex(s) to destination vertex(v)
        int sd=0; //denotes the length of the shortest path from s to v
        int count =0; //denotes the number of vertices in that shortest path

        while(v!=s){ //creates the path  by following the predacessors until we reach the source vertex
            count++;
            path[count] = v;
